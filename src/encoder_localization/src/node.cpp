...
class EncoderLocalizationNode : public rclcpp::Node {
 ...
 private:
  ...
  void callback(const EncoderCounts::ConstSharedPtr msg) {
    ...
    pose.covariance[0] = 1e-2;
    pose.covariance[7] = 1e-2;
    pose.covariance[14] = 0;
    pose.covariance[21] = 0;
    pose.covariance[28] = 0;
    pose.covariance[35] = 0.03045025;

    twist.covariance[0] = 1e-2;
    twist.covariance[7] = 1e-2;
    twist.covariance[14] = 0;
    twist.covariance[21] = 0;
    twist.covariance[28] = 0;
    twist.covariance[35] = 0.03045025;

    RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                         " pose: %lf, %lf, %lf", pose.pose.position.x,
                         pose.pose.position.y,
                         tf2::getYaw(pose.pose.orientation));
    RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                         "twist: %lf, %lf, %lf, dt: %f", twist.twist.linear.x,
